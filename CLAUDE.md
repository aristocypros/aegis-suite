...
**OPA distribution is BUNDLE PULL, not push.** The backend assembles one gzipped-tar bundle (`opa/config.yaml` → `GET /bundle/aegis.tar.gz`) that every OPA replica polls and converges on. This is what lets you run multiple stateless OPA replicas with consistent enforcement — see [services/opaBundle.js](backend/src/services/opaBundle.js) and the "OPA state distribution" section below. The services that run under `docker-compose.yml`:

| Service           | Port            | Role                                                                                          |
|-------------------|-----------------|-----------------------------------------------------------------------------------------------|
| `opa`             | 8181            | OPA server. Boots with ONLY `opa/config.yaml` (`--config-file --authentication=token --authorization=basic`) and **pulls** a bearer-authed bundle from the backend's `/bundle` endpoint every 10-20s. No policies/data/trust files are mounted — authz Rego + policies + `data.studio.*` + `data.platform_keys` all arrive in the bundle. `depends_on: backend (healthy)`. |
...
The backend does **not** push policies/data into OPA. It assembles a single gzipped-tar bundle in [services/opaBundle.js](backend/src/services/opaBundle.js) and serves it at `GET /bundle/aegis.tar.gz`; every OPA replica polls it (`opa/config.yaml`, 10-20s, ETag/304) and converges. This is the design that lets you run **multiple stateless OPA replicas** with consistent enforcement — the old push-to-one-URL model left every other replica stale.

- **Bundle contents:** `system_authz.rego` + `studio_authz.rego`, every active (non-locked) compiled policy module, and the data docs `studio/policy_index`, `studio/keys`, `studio/callers`, `studio/caller_access`, `platform_keys`. A `.manifest` declares explicit `roots` (the data roots + each user policy package), recomputed every build, deliberately EXCLUDING the `__preview_`/`__validate_` namespaces so those stay pushable (see below). Revision/ETag is a sha256 of the canonical logical inputs (deterministic — `buildPolicyIndex` emits `generatedAt: null` so an unchanged policy set yields a 304).
- **Cache + invalidation:** the bundle is cached in-process and lazily rebuilt. Every mutation that used to call a `publish*` function now calls `invalidateBundle(reason)` (a thin wrapper over `opaBundle.invalidateBundle`) — same fire-and-forget call sites in [server.js](backend/src/server.js); the next poll rebuilds once, batching bursts. The router factories (`createTrustKeysRouter`, `createPepCallersRouter`, …) still take a `publish` callback — it's now `invalidateBundle`.
- **Endpoint auth (SECURITY):** the bundle carries cleartext HMAC secrets (caller + trust-key secrets), so `/bundle` is bearer-authed with a constant-time compare against `BUNDLE_TOKEN_FILE` (`opa_bundle_token`, minted by vault-init). It is mounted BEFORE `app.use(authenticate)` (the puller is OPA, not a user session), fail-closed (503 if no token configured), `Cache-Control: no-store`, never logged, and must stay internal-only — **never** proxy `/bundle` through nginx.
- **The two writes that REMAIN pushes:** `/api/preview-evaluate` pushes a transient module under a `__preview_<ts>` package, and policy create/update probe-compiles the Rego under a throwaway `__validate_<ts>` package (then deletes it) so a module OPA can't parse is rejected 400 at write-time rather than bricking the whole bundle's activation. Both packages are OUTSIDE every bundle root (OPA only forbids writes to bundle-owned paths), which is why the reserved-prefix guard in `regoCompiler.js` (`reservedPackageRootError`) refuses user packages whose first segment is `system`/`studio`/`platform_keys` or starts with `__preview_`.
- **Eventual consistency:** changes propagate within one poll (~10-20s) plus the PEP's own caller-cache TTL. **Policy lock is therefore EVENTUAL, not an instant kill-switch** — a bundle-owned module cannot be REST-deleted, so there is no per-replica instant default-deny like the old push model had. Shorten the poll in `opa/config.yaml` if a faster kill matters.
- **Boot ordering is inverted:** OPA `depends_on: backend (healthy)` and pulls the bundle; the backend serves it without needing OPA. The backend's `GET /healthz` (gated on bundle warm) is the readiness signal. The `opa-trust-init` sidecar + `opa_trust` volume were removed — `platform_keys` is built into the bundle from KMS+DB.
